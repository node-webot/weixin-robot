...
function Info(json){
  ...
  this.event = json.Event && String(json.Event);
  this.eventKey = json.EventKey && String(json.EventKey);
...
/**
 * @cfg {String} 事件类型，subscribe(订阅)、unsubscribe(取消订阅)、CLICK(自定义菜单点击事件)
 * 对应于原始字段: Event
 */
Info.prototype.event = null;

/**
 * @cfg {String} 事件KEY值，与自定义菜单接口中KEY值对应
 * 对应于原始字段: EventKey
 */
Info.prototype.eventKey = null;
...
Info.pack = function(info, mapping){
  ...
  // 具备图文消息的特征
  if (_.isObject(reply) && Object.keys(reply).length > 3) {
